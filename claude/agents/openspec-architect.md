...
- `shared/quality-framework.md` 提供质量门禁体系
...
| 发现的问题类型 | 回退阶段 | 原因 |
|----------------|---------|------|
| 业务目标不清、约束未识别、必要性验证失败 | Phase 1 | 需求理解错误 |
| 候选方案不足（<2个）、对比维度缺失 | Phase 2 | 方案探索不充分 |
| 推荐方案与 Phase 1 底层问题不匹配 | Phase 1 | 需求理解偏离 |
| 设计遗漏关键场景（异常/并发/边界） | Phase 2 | 设计不完整 |
| 任务拆解与设计不一致 | Phase 2 | 设计细节缺失 |
| 验收标准无法验证设计目标 | Phase 2 | 设计目标模糊 |
