...
4. **自动分级判定**（新增）：
   ...
   读取生成的 `.classification.json`，向用户展示分级结果：
   
   ```
   📊 自动分级结果：L2（标准门禁）
   建议跑的维度：architecture + database + security + performance
   预计耗时：3-6 分钟
   ...
   - 用户确认 `Y` → 按建议的维度执行
   - 用户输入 `n` → 询问期望的等级（L0/L1/L2/L3），或手动指定维度
   - 若判定为 L0，提示可豁免门禁直接 apply（但需在 `review-summary.md` 记录豁免理由）
   
   **拿不准就升一级**：如果用户表示不确定，建议升级到更高等级。
...
2. **执行误报检测**（新增）：按 `shared/false-positive-detection.md` 规则检测可能的误报或过度建议。生成"疑似误报检测"区块，列出需要人工复核的 finding。
3. **门禁裁决**：判定规则见 `shared/gate-policy.md`。**注意**：在计算裁决时，应考虑误报检测建议降级后的级别（如果用户选择应用自动降级）。
4. **各维度结论一览表**：维度 | 结论 | Blocker 数 | Major 数 | Minor 数 | 本轮重跑/沿用上轮。
5. **疑似误报检测**（新增区块）：列出可能的误报、过度建议、跨维度矛盾等，供人工复核。包含"自动降级建议"和"需人工判断"两类。
6. **已确认风险**：按维度 + 严重级别汇总。
7. **修改建议**：合并各维度建议，去重，标注需在 `design.md` / `tasks.md` 落实的项。
8. **跨维度冲突清单（交叉验证核心产出）**：不同维度的建议互相矛盾时列出（如性能建议与安全建议冲突）。**冲突项必须在 `design.md` 中闭环，不能让矛盾的建议同时进入 `tasks.md`。**
9. **「有条件通过」的条件清单**：`条件ID | 来源维度 | 条件内容 | 对应 tasks.md 任务 | 状态`。**映射不到 tasks 的条件视同 Blocker。**
10. **上轮闭环验证结果**（仅重走门禁时）：哪些历史 finding 已闭环、哪些声称已闭环但实际未闭环。后者一律按未闭环 Blocker 计入裁决。
11. **术语表**：只列本次评审实际出现的专有名词 → 白话解释。这是让签字人真正读懂 Blocker 的前提。
12. **人工确认区**：留一行 `Technical Review Approved: __________`（待人工填写），注明批准前禁止 apply。
